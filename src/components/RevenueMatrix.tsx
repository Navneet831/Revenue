...
export const RevenueMatrix: React.FC = () => {
    ...
    return (
        <div
            id="matrix-container"
            className="flex-1 w-full flex flex-col overflow-hidden bg-transparent"
        >
            {/* ─── FIXED HEADERS (Month Selectors) ───────────────────────────── */}
            <div className="w-full overflow-x-auto no-scrollbar shrink-0 z-40 bg-[#141b2d]">
                <table className="w-full border-collapse min-w-full" style={{ tableLayout: 'fixed' }}>
                    <thead>
                        <tr>
                            <th
                                className={`${thBase} border-r border-slate-800 sticky left-0 z-50`}
            ...
            {/* ─── SCROLLABLE CONTENT (Table Rows OR Chart) ─────────────────── */}
            <div className="flex-1 w-full overflow-y-auto overflow-x-hidden custom-scrollbar bg-[#0b101e]">
                {isVisual ? (
                    <div className="h-full min-h-[380px] w-full pt-4 px-2 relative">
