...
export const RevenueDashboard: React.FC = () => {
    ...
    return (
        <div className="flex flex-col min-h-full">
            <header className="bg-[#0F1219] border-b border-slate-800 flex flex-col lg:flex-row justify-between items-start lg:items-center px-4 py-2 shrink-0 z-40 relative gap-3 min-w-0 w-full max-w-full">
                <div className="flex items-center gap-3 overflow-x-auto flex-1 min-w-0 no-scrollbar pb-1 lg:pb-0 w-full">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        <div className="flex items-center bg-[#111620] rounded-md overflow-hidden btn-3d">
                            <input type="date" value={filters.startDate || ''} onChange={(e) => updateFilters({ startDate: e.target.value })} className="bg-transparent text-slate-400 text-[10px] p-1.5 outline-none font-mono" />
                        </div>
                        <span className="text-slate-600 text-xs font-bold">-</span>
                        <div className="flex items-center bg-[#111620] rounded-md overflow-hidden btn-3d">
                            <input type="date" value={filters.endDate || ''} onChange={(e) => updateFilters({ endDate: e.target.value })} className="bg-transparent text-white text-[10px] p-1.5 outline-none font-mono" />
                        </div>
                    </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                    <div className="flex items-center bg-[#0A0C10] rounded-full p-[3px] border border-slate-800">
                        {(['Amount', 'MW', 'Qty'] as const).map(m => (
                            <button key={m} onClick={() => handleMetricChange(m)} className={`px-4 py-1 text-[10px] font-extrabold rounded-full transition-all uppercase ${filters.metric === m ? 'bg-emerald-500 text-black shadow-[0_2px_8px_rgba(16,185,129,0.4)]' : 'text-slate-500 hover:text-slate-300'}`}>{m}</button>
                        ))}
                    </div>
                    <button onClick={() => updateUIState({ insightsOpen: !ui.insightsOpen })} className={`flex items-center justify-center transition-all ${!insightsSeen ? 'text-amber-400 scale-110' : 'text-slate-500 hover:text-amber-400'}`}>
            ...
            {isError && (
                <div data-testid="data-health-banner" className="mx-3 mt-3 flex items-center justify-between gap-3 bg-rose-950/40 border border-rose-900/50 rounded-xl px-4 py-2">
                    <div className="flex items-center gap-2 min-w-0">
                        <AlertTriangle className="w-4 h-4 text-rose-400 shrink-0" />
                        <span className="text-[10px] font-bold text-rose-300 uppercase tracking-widest shrink-0">Data Feed Degraded</span>
                        <span className="text-[10px] text-slate-500 font-mono truncate">{(error as Error)?.message}</span>
                    </div>
                    <button onClick={() => refetch()} className="flex items-center gap-1.5 px-3 py-1 bg-[#1E293B] text-slate-300 hover:text-white rounded-md text-[10px] font-bold uppercase tracking-wider shrink-0">
            ...
            <div className="flex-1 px-3 pb-3 flex flex-col gap-4 w-full">
                <div id="w-master" className="w-full card-3d bg-[#111620] rounded-2xl border border-slate-800 flex flex-col overflow-hidden relative shrink-0" style={{ height: '345px' }}>
                    ...
                    <div className="p-1 px-3 border-b border-slate-800 bg-[#0F1219] flex justify-between items-center z-50 shrink-0 h-9">
                        <div className="flex items-center gap-2 overflow-hidden flex-1">
                            <div className="flex items-center shrink-0">
                                ...
                                <span className="text-[10px] font-bold text-white uppercase tracking-tight whitespace-nowrap">{cardViews.master === 'visual' ? 'Velocity' : 'Matrix'}</span>
                            </div>
                            <SkuLegend />
                        </div>
                        <div className="flex items-center gap-3 shrink-0 ml-2">
                            ...
                            <button onClick={() => setCardView('master', cardViews.master === 'visual' ? 'tabular' : 'visual')} className="p-1 px-2 btn-3d bg-[#1E293B] text-slate-300 hover:text-white rounded-md">
