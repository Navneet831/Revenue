...
Usage:  python build_exe.py [--full] [--skip-build]

Stages the production-relevant files then runs PyInstaller --onedir.
The output runs WITHOUT admin rights on any standard Windows user account.

Flags:
  --full        Force re-copy of node_modules (slow, ~200 MB).
  --skip-build  Skip npm build steps (use existing dist/).
...
def step_stage(full: bool):
    ...
    # Windows manifest (asInvoker — no admin prompt)
    if MANIFEST.exists():
        shutil.copy2(MANIFEST, STAGING / 'app.manifest')
        print('  copied  app.manifest')
    else:
        print('  WARNING: app.manifest not found — exe may prompt for elevation')

    # .env  (DB creds + feature flags — required by env.js via ../../.env)
    shutil.copy2(ROOT / '.env', STAGING / '.env')
    print('  copied  .env')

    # Backend source  (no tests, no node_modules)
    sync_dir(
        ROOT / 'apps' / 'api',
        STAGING / 'apps' / 'api',
...
def step_write_spec():
    ...
    manifest_line = ''
    staged_manifest = STAGING / 'app.manifest'
    if staged_manifest.exists():
        # Use forward slashes inside the spec string
        manifest_line = f"    manifest='app.manifest',"
...
a = Analysis(
    ...
    # pywebview (pythonnet/WinForms backend) hidden imports
    hiddenimports=[
        'webview',
...
def main():
    ...
    if not skip_build:
        ...
        print('\n[0] Skipping npm build (--skip-build).')
