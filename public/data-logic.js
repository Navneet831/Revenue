...
exports.DataLogic = exports.RevenueComputeEngine = exports.ConcentrationAnalyser = exports.DataSanitizer = exports.ChronologicalIndexer = exports.Format = exports.MetricFormatter = exports.CONFIG = exports.InsightScorer = exports.AdvancedQuantEngine = void 0;
/**
...
class AdvancedQuantEngine {
     ...
     */
    static calculatePriceEntropy(data) {
        if (data.length < 5)
            return 0;
     ...
     */
    static calculateDemandSlope(data) {
        if (data.length < 2)
            return 0;
        const n = data.length;
...
exports.AdvancedQuantEngine = AdvancedQuantEngine;
/**
 * Insight Scoring Framework (ISF)
 * Score = Impact + Novelty + Confidence + User Relevance + Recency
 */
class InsightScorer {
    static score(insight, impactMagnitude, recencyDays) {
...
exports.InsightScorer = InsightScorer;
exports.CONFIG = {
    SHEET_ID: null,
    SHEET_NAME: 'revenue',
...
class RevenueComputeEngine {
    ...
    compute() {
        ...
        for (let j = 0; j < rangeRecords.length; j++) {
            ...
            if (!buckets.chart.monthly[mName][plotKeyChart])
                buckets.chart.monthly[mName][plotKeyChart] = 0;
            buckets.chart.monthly[mName][plotKeyChart] += metricVal;
            if (r.week <= 5) {
                if (!buckets.chart.weekly[mName][r.week][plotKeyChart])
                    buckets.chart.weekly[mName][r.week][plotKeyChart] = 0;
                buckets.chart.weekly[mName][r.week][plotKeyChart] += metricVal;
            }
            if (r.day <= 31) {
                if (!buckets.chart.daily[mName][r.day - 1][plotKeyChart])
                    buckets.chart.daily[mName][r.day - 1][plotKeyChart] = 0;
                buckets.chart.daily[mName][r.day - 1][plotKeyChart] += metricVal;
            }
            const qIdx = qtrMap[r.monthIdx];
            if (qIdx !== undefined) {
                if (!buckets.chart.quarterly[qIdx][plotKeyChart])
                    buckets.chart.quarterly[qIdx][plotKeyChart] = 0;
        ...
        const finalizedActivePlotKeys = Array.from(activePlotKeys).filter(key => {
            ...
            const hasWeekly = Object.values(buckets.chart.weekly).some(m => Object.values(m).some(w => w[key] > 0));
            const hasDaily = Object.values(buckets.chart.daily).some(d => d[key] > 0);
